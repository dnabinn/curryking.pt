...
(function () {
    ...
    /* ----------------------------------------------------------
       9. FULL MENU — Tab switcher
       Toggles between the "Menu Pages" (image grid) panel
       and the "PDF View" (iframe embed) panel.
    ---------------------------------------------------------- */
    const fmenuTabs    = document.querySelectorAll('.fmenu__tab');
    const fmenuPanels  = document.querySelectorAll('.fmenu__panel');

    fmenuTabs.forEach(function (tab) {
        tab.addEventListener('click', function () {
            const targetId = 'tab-' + tab.getAttribute('data-tab');

            // Update tab active state
            fmenuTabs.forEach(function (t) {
            ...
            // Show / hide panels
            fmenuPanels.forEach(function (panel) {
                if (panel.id === targetId) {
                    panel.classList.remove('fmenu__panel--hidden');
                } else {
                    panel.classList.add('fmenu__panel--hidden');
                }
            });
        });
    });
    ...
    /* ----------------------------------------------------------
       10. LIGHTBOX — opens when a .fmenu__page card is clicked
       Supports:
    ...
    // State
    let lbPages    = [];   // array of { src, alt } objects
    let lbCurrent  = 0;    // current index

    /* Collect all menu pages into lbPages array.
       Called once on load and again if pages are dynamically added. */
    function collectPages() {
        lbPages = [];
        document.querySelectorAll('.fmenu__page').forEach(function (page) {
            const img = page.querySelector('img');
            if (img) {
                lbPages.push({
                    src: img.src,
                    alt: img.alt || 'Menu page'
                });
            }
        });
    }
    ...
    function openLightbox(index) {
        ...
        lbCurrent = Math.max(0, Math.min(index, lbPages.length - 1));
    ...
    /* Load image at index with fade transition */
    function loadImage(index) {
        if (!lbPages.length) return;
        lbCurrent = Math.max(0, Math.min(index, lbPages.length - 1));

        const page = lbPages[lbCurrent];

        // Start fade-out + show spinner
        ...
        preload.onload = function () {
            ...
            // Brief delay so CSS transition fires after src change
            requestAnimationFrame(function () {
                requestAnimationFrame(function () {
        ...
        // Update counter
        lightboxCounter.textContent = 'Page ' + (lbCurrent + 1) + ' of ' + lbPages.length;

        // Disable prev/next at boundaries
        lightboxPrev.disabled = (lbCurrent === 0);
        lightboxNext.disabled = (lbCurrent === lbPages.length - 1);
    }

    /* Navigate */
    function lightboxGo(delta) {
        loadImage(lbCurrent + delta);
    }

    /* Attach click listeners to each menu page card */
    function bindPageClicks() {
        document.querySelectorAll('.fmenu__page').forEach(function (page, index) {
            // Clicking the card or the zoom button opens the lightbox
            page.addEventListener('click', function () { openLightbox(index); });
        });
    }

    // Bind on load (and again if cards are added dynamically via JS)
    bindPageClicks();
    ...
    (function () {
        ...
        lightbox.addEventListener('touchend', function (e) {
            ...
            // Only trigger on predominantly horizontal swipe (≥40px, not too vertical)
